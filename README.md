...
| **画布图例（模式语义）** | 画布左上角写明「这张图在当前模式下怎么读」：主持人统筹=逐次派发、多模型平等=专家直达互辩、针锋相对=只挑毛病；系统补出来的骨架通道会标注「虚线 = 骨架通道，不是真实流转」。同一张图在三种模式下含义不同，图例把它写在图上是刻意的——不写清楚，用户会把它读成一张控制流程的路径图。 |
| **节点状态如实显示** | 节点直接显示持久子代理的真实状态：**工作中 / 空闲（可续聊）/ 就绪（可唤醒）/ 未唤醒 / 已退出**。刻意**不发明**「已完成 / 已失败」——子代理跑完一轮回到空闲，仍可被再次唤醒，说它"结束了"是错的。 |
